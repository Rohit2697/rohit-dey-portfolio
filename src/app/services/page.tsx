    ...
    >
      ...
      <div className="relative z-10 w-full px-8 lg:px-16">
        <div className={`flex ${section.contentPosition === "right" ? "justify-end" : "justify-start"}`}>
          <div
            className={`max-w-3xl transform transition-all duration-1000 ease-out ${isVisible
            ...
            <div className="bg-black/80 backdrop-blur-md rounded-2xl p-8 lg:p-12 shadow-2xl border border-white/20">
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-4">
                  <span className="text-xl font-semibold text-green-400">{section.service.role}</span>
                ...
                <div className="text-white/80 mt-2">
                  ...
                  {formatDate(section.service.joiningDate)} - {formatDate(section.service.lwd)}
                  <span className="ml-2 text-green-400">
                    ({calculateDuration(section.service.joiningDate, section.service.lwd)})
                  </span>
                </div>
              </div>
              ...
              {/* Projects */}
              {section.service.projects && section.service.projects.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-xl font-semibold text-green-400 mb-3">Projects</h3>
                  {section.service.projects.map((clientProject, clientIndex) => (
                    <div key={clientIndex} className="mb-4">
                      <h4 className="text-lg font-medium text-white mb-2">
                        <span className="text-green-400">Client:</span> {clientProject.client}
                      </h4>
                      <div className="space-y-3 ml-4">
                        {clientProject.projects.map((project, projectIndex) => (
                          <div key={projectIndex} className="border-l-2 border-green-500 pl-4">
                            <h5 className="font-semibold text-green-400 mb-1">{project.name}</h5>
              ...
              {/* Recognitions */}
              {section.service.recognitions && section.service.recognitions.length > 0 && (
                <div>
                  <h3 className="text-xl font-semibold text-green-400 mb-3">Recognitions</h3>
                  <div className="flex flex-wrap gap-2">
                    {section.service.recognitions.map((recognition, index) => (
                      <span
                        key={index}
    ...
    <div className="overflow-hidden">
      {sections.map((section) => (
        <div
          ...
          className="section"
        >
          <ServiceSection section={section} isVisible={visibleSections.has(section.id)} />
        </div>
      ))}
    </div>
  )
